...
def _resolve_range(from_pointer, to_pointer, repo_dir=None, verbose=False):
    ...
    if verbose:
        print(f"Resolving 'from' pointer: {resolved_from}", file=sys.stderr)
    from_sha = resolve_commit_pointer(resolved_from, repo_dir=repo_dir)
    if verbose:
        print(f"  -> {from_sha}", file=sys.stderr)

    if verbose:
        print(f"Resolving 'to' pointer: {resolved_to}", file=sys.stderr)
    to_sha = resolve_commit_pointer(resolved_to, repo_dir=repo_dir)
    if verbose:
        print(f"  -> {to_sha}", file=sys.stderr)

    if from_sha == to_sha and verbose:
        print("Warning: 'from' and 'to' point to the same commit. Generating diff anyway.", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    # Output markdown to stdout for use by the skill
    markdown = make_diff_string(
